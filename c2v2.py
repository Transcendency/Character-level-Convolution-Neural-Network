...
class Char2Vec(object):
	...
	def build_dataset(self,words):
		...
		count.extend(collections.Counter(words).most_common(self.op.vocabulary_size - 1))
		dictionary = dict()
		for word, _ in count:
	...
	def generate_batch(self,batch_size, num_skips, skip_window,words):
	  ...
	  for _ in range(span):
	    ...
	    selfdata_index = (self.data_index + 1) % len(self.data)
	...
	def train(self):
		...
		vocabulary_size = self.op.vocabulary_size
		...
		save_path = saver.save(session, "cache")


def main():
#	opts = Options()
#	opts.set_vocabulary_size(1700000)
