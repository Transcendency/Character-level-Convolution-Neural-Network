...
class Char2Vec(object):
	...
	def generate_batch(self,batch_size, num_skips, skip_window,words):
	  ...
	  words = [self.reverse_dictionary[index] for index in batch]
	  return words, labels, batch
	...
	def train(self):
		...
		# index_inputs = tf.placeholder(tf.int32,shape=[batch_size])
		######################Graph##############################################
		...
		features = tf.concat(1,[self.convolution(embeddings,kenels[k],train_inputs,kenel_height[k],batch_indexes) for k in range(len(kenels))])
		...
		for step in xrange(num_steps):
			word_batch, batch_labels, batch_index = self.generate_batch(batch_size, num_skips, skip_window, words)
			...
			feed_dict = {train_inputs : batch_inputs, train_labels : batch_labels,learning_rate :lr, index_inputs:batch_index}
